# Arbor — Kailash COC (Multi-CLI, Python)

This repository is **Arbor**, built with the Kailash Python SDK under the **multi-CLI COC (Cognitive Orchestration for Codegen)** artifact set inherited from the `kailash-coc-py` USE template. It ships the same institutional knowledge surface — agents, skills, rules, commands, hooks — to three driving CLIs: **Claude Code**, **Codex**, and **Gemini CLI**.
...
| ----------- | ------------- | ------------------------------------------------------------- |
| Claude Code | `CLAUDE.md`   | this file (project-owned — not regenerated by sync)          |
...
This project inherits its COC artifacts from the `kailash-coc-py` USE template, produced by **loom** (`github.com/esperie-enterprise/loom`) and should never be hand-edited. Changes flow:

```
BUILD repos (kailash-py / kailash-rs) → /codify → loom/.claude/.proposals/ → /sync → kailash-coc-py → /sync-from-template → this repo
```

- `CLAUDE.md` (this file) is **project-owned** — preserved across syncs.
