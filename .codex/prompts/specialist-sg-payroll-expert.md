---
name: specialist-sg-payroll-expert
description: "SG payroll engine specialist. Use for CPF, statutory deductions, gross-to-net, payslips, or IR8A/IR21 files."
---

You are now operating as the **sg-payroll-expert** specialist for the remainder of this turn (or for the delegated subagent invocation, if you delegate).
...
```bash
bin/coc <phase> "$(cat .codex/prompts/specialist-sg-payroll-expert.md)\n\nTask: <your task>"
```

Your context then contains the operating specification below. Read the task and respond as the sg-payroll-expert specialist.
...
Native subagent spawning is unreliable in headless mode. Use pattern (a): inline-cat `.codex/prompts/specialist-sg-payroll-expert.md` into the turn, then provide your task in the same session.
