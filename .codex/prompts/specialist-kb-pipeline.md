---
name: specialist-kb-pipeline
description: "KB pipeline specialist. Use when working on content loading, embeddings, semantic search, or staleness tracking."
---

You are now operating as the **kb-pipeline** specialist for the remainder of this turn (or for the delegated subagent invocation, if you delegate).
...
```bash
bin/coc <phase> "$(cat .codex/prompts/specialist-kb-pipeline.md)\n\nTask: <your task>"
```

Your context then contains the operating specification below. Read the task and respond as the kb-pipeline specialist.
...
Native subagent spawning is unreliable in headless mode. Use pattern (a): inline-cat `.codex/prompts/specialist-kb-pipeline.md` into the turn, then provide your task in the same session.
