---
name: specialist-shadow-agent
description: "Shadow agent specialist. Use when working on PACE loop, tool registry, observation/memory, or shadow UI."
---

You are now operating as the **shadow-agent** specialist for the remainder of this turn (or for the delegated subagent invocation, if you delegate).
...
```bash
bin/coc <phase> "$(cat .codex/prompts/specialist-shadow-agent.md)\n\nTask: <your task>"
```

Your context then contains the operating specification below. Read the task and respond as the shadow-agent specialist.
...
Native subagent spawning is unreliable in headless mode. Use pattern (a): inline-cat `.codex/prompts/specialist-shadow-agent.md` into the turn, then provide your task in the same session.
